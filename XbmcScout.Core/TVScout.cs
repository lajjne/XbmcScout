    ...
    public class TVScout {
        ...
        public void ProcessSeason(String directory, String seasonFldr, int seasonNum, int level) {
            ...
            if (options.GetSeasonPosters) {
                ...
                if (options.SaveMyMoviesMeta)
                    SaveImage(directory + "\\" + seasonFldr, "backdrop.jpg", null, 0, seasonNum.ToString(), XbmcScout.Core.Providers.TVShowPosterType.Season_Backdrop, level);
            }
            ...
            if (options.DownloadAllSeasonBackdrops) {
                ...
                foreach (Posters p in seasonBackdrops) {
                    ...
                    if (options.SaveMyMoviesMeta)
                        SaveImage(directory + "\\" + seasonFldr, "backdrop" + i + ".jpg", seasonBackdrops, i, seasonNum.ToString(), XbmcScout.Core.Providers.TVShowPosterType.Season_Backdrop, level + 1);
                    i++;
                }
            }
            #endregion
